...
export default function QuestionCard({ question, questionNumber, options, type, min, max, helperText, info, image, onSelect, onBack, onClose }) {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full min-h-[100dvh] md:h-screen bg-[#111827] md:bg-white md:overflow-hidden relative">
      ...
      {onClose && (
        <button
          onClick={onClose}
          className="absolute right-4 top-5 md:right-8 md:top-8 p-3 text-white/80 hover:text-white bg-black/20 hover:bg-black/40 backdrop-blur-md rounded-full transition-all z-30"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-5 h-5">
